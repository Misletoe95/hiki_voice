...
PAN_LEFT_ZOOM_OUT = 61  #云台左转和焦距变小(NET_DVR_AUDIOENC_INFO倍率变小)
...
# 设备参数结构体 V30
# class NET_DVR_AUDIOENC_INFO(ctypes.Structure):
#     _fields_ = [
    
#     ("in_frame_size",c_byte),
#     ("reserverd",c_int16),   
#     ]
...
#语音解码信息结构体
class NET_DVR_AUDIODESC_INFO(Structure):
    _fields_ = [
        ("nchans",c_int),
        ("sample_rate",c_int),
...
LPNET_DVR_AUDIODESC_INFO = POINTER(NET_DVR_AUDIODESC_INFO)
#语音解码结构体
class NET_DVR_AUDIODEC_PROCESS_PARAM(Structure):
    #info = NET_DVR_AUDIODESC_INFO()
    _fields_  = [
        ("in_buf",POINTER(c_ubyte)),
        ("out_buf",POINTER(c_ubyte)),
        ("in_data_size",c_ulong),
        ("proc_data_size",c_ulong),
        ("out_frame_size",c_ulong),
        ("dec_info",NET_DVR_AUDIODESC_INFO),
        ("g726_dec_reset",c_int),
        ("g711_type",c_int),
        ("reserved",c_int*16)
    ]
# 定义预览参数结构体
...
NET_DVR_PREVIEWINFO._fields_ = [
    ...
    ('hPlayWnd', c_uint32),  # 播放窗口的句柄,为NULL表示不播放图象NET_DVR_StartVoiceCom_V30
...
class NET_DVR_ACS_EVENT_INFO(Structure):
    _fields_ = [
        ...
        ("byCardType", c_byte),  # 卡类型：1- 普通卡，2- 残障人士卡，3- 非授权名单卡，4- 巡更卡，5- 胁迫卡，6- 超级卡，7- 来宾卡，8- 解除卡，为0表示无效
...
class NET_DVR_ID_CARD_INFO_ALARM(Structure):
    _fields_ = [
        ...
        ("byCardType", c_ubyte),  # 卡类型，1-普通卡，2-残疾人卡，3-非授权名单卡，4-巡更卡，5-胁迫卡，6-超级卡，7-来宾卡，8-解除卡，为0无效
        ("byDeviceNo", c_ubyte),  # 设备编号，为0时无效（有效范围1-255）
        ("byMask", c_ubyte),  # 是否带口罩：0-保留，1-未知，2-不戴口罩，3-戴口罩
        ("byRes2", c_ubyte),  # 保留，置为0
        ("dwFingerPrintDataLen", c_uint32),  # 指纹数据大小，不为0是表
...
#实时语音回调
REALVOICEDATACALLBACK = fun_ctype(None, c_long,POINTER(c_char),c_ulong,c_ubyte, c_void_p)
MRVOICEDATACALLBACK = fun_ctype(None, c_long,POINTER(c_char),c_ulong,c_ubyte, c_void_p)